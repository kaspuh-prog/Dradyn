...
## Godot 4.5 — Derived-from-GDD version
...
# --- NEW: MP derivation policy hooks (read by DerivedFormulas.mp_max via duck-typing) ---
# Valid values: "", "int", "wis", "hybrid"
# "" (empty) = legacy behavior (both INT and WIS contribute)
@export var _mp_source_policy: String = ""
@export var _mp_hybrid_weights: Dictionary = {"INT": 0.5, "WIS": 0.5}

# Provide public getters for duck-typing (DerivedFormulas checks these).
...
# Optional setter for runtime class swaps (LevelComponent will call this later).
func set_mp_source(source: String, weights: Dictionary = {}) -> void:
	...
	match source:
		...
		_:
			_mp_source_policy = ""  # legacy/fallback
...
@export var stamina_regen_per_sec: float = 0.0 : set = _set_regen_stam
...
func _set_regen_mp(v: float) -> void:
	mp_regen_per_sec = v
	_recalc_processing()
func _set_regen_stam(v: float) -> void:
	stamina_regen_per_sec = v
	_recalc_processing()
...
func _get_base_from_resource(stat_key: String, default_val: float = 0.0) -> float:
	...
	# Try StatsResource.base_stats (Dictionary)
	...
	# Fallback: direct property on the Resource
...
func _derived_base_for(stat_name: String) -> float:
	...
	match stat_name:
		"END":
			return DerivedFormulas.end_from_sta(self) if derive_end_from_sta else get_base_stat("END")
		"HP":
...
func get_all_final_stats() -> Dictionary:
	...
	if typeof(dict_v) == TYPE_DICTIONARY:
		...
		var keys := [
			"HP","MP","Stamina","MoveSpeed","Attack","Defense",
			"STR","DEX","STA","INT","WIS","CHA","LCK","END",
...
func max_stamina() -> float: return get_final_stat("Stamina")
...
func spend_stamina(amount: float) -> bool:
	if amount <= 0.0: return true
	if current_stamina + 1e-6 < amount: return false
...
func restore_stamina(amount: float) -> void:
	if amount > 0.0: change_stamina(amount)
...
func _get_resistance_for(type_name: String) -> float:
	...
	var r: float = get_final_stat(key) if key != "" else 0.0
...
func apply_damage_packet(packet: Dictionary) -> void:
	...
	# Base portion (Defense only)
	if base > 0.0:
		total += _mitigate_by_defense(base, def_val)

	# Typed portions (Defense then Resistance)
	if types.size() > 0:
		for t in types.keys():
...
func _reconcile_all_vitals() -> void:
	...
	_reconcile_vital_if_needed("Stamina")
...
func _process(delta: float) -> void:
	...
	var st_r: float = (DerivedFormulas.stamina_regen_per_sec(self) if use_gdd_derived else stamina_regen_per_sec)

	if hp_r > 0.0: change_hp(hp_r * delta)
	if mp_r > 0.0: change_mp(mp_r * delta)
	if st_r > 0.0: change_stamina(st_r * delta)

# --- Debug print on enter (handy when tuning) ---
...
# --- NEW: allocation helper (blocks HP/MP/END as requested) ---
