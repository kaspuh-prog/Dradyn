...
@onready var _pm: Node = null

func _resolve_party_manager() -> Node:
	# Canonical path: autoload singleton configured in project.godot.
	# This avoids ambiguous lookup when scene nodes also use "PartyManager" group.
	var by_autoload: Node = get_node_or_null("/root/Party")
	if by_autoload != null:
		return by_autoload

	# Soft fallback for legacy scenes where Party autoload is missing.
	var by_group: Node = get_tree().get_first_node_in_group("PartyManager")
	if by_group != null:
		push_warning("[PartyRoot_Bootstrap] /root/Party missing; falling back to PartyManager group lookup.")
	return by_group

func _ready() -> void:
	_pm = _resolve_party_manager()
